...
def minimize_app():
    root.iconify()  # Minimize the application window

# Create a rounded button function
...
# Create a frame for the user authentication
...
# Create registration and login UI
def create_auth_ui():
    # Load logo image
...
# Call the function to create authentication UI
create_auth_ui()    
        ...
        if exercise_data:
            ...
            try:
                for i, muscle in enumerate(muscle_groups.keys()):
                    current_session['muscles'][muscle] += int(intensities[i])
            ...
            print(name)# Convert intensity to int
            description = get_exercise_description(name)
            print(description)
def get_exercise_description(exercise_name):
    try:
        with open('exercise_notes.txt', 'r') as file:
        ...
        for line in exercises:
            if line.lower().startswith(exercise_name):  # Compare in lowercase
                return line.strip()  # Return the found line
...
# Function to set up workout tracker UI
